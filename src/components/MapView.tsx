...
export default function MapView() {
  ...
  const buildSeriesData = useCallback(() => {
    ...
    return Object.entries(colorMap).map(([code, color]) => {
      const feature = geoJsonRef.current.features.find(
        (f: any) => String(f.properties.adcode) === code
      );
      return {
      ...
      try {
        ...
        chart.setOption({
            ...
            formatter: (params: any) => {
              ...
              if (city && city.visits.length > 0) {
                const lines = city.visits
                  .map((v) => `${VISIT_TYPE_META[v.type]?.emoji} ${v.year}年 · ${VISIT_TYPE_META[v.type]?.label}`)
                  .join('<br/>');
          ...
          series: [{
            ...
            itemStyle: { areaColor: '#F0F0F0', borderColor: '#D8D8D8', borderWidth: 0.6 },
  ...
  useEffect(() => {
    if (chartRef.current && geoJsonRef.current) {
      chartRef.current.setOption({ series: [{ data: buildSeriesData() }] });
    }
  }, [buildSeriesData]);
